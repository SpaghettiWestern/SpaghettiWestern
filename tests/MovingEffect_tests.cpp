...
//	MovingEffect(ScreenCoordinate loc);
	TEST(constructor_location){
		Coordinate2D<double> loc(1.0,28.0);
		...
		CHECK(test_effect.startMove() == false);
		CHECK(test_effect.getType() == MOVINGEFFECT);
		CHECK(test_effect.getNextStep() == Coordinate2D<double>(-1.0,-1.0));
	}
...
//	MovingEffect(ScreenCoordinate loc, const std::vector<ScreenCoordinate>& path);
	TEST(constructor_loc_path){
		Coordinate2D<double> s_loc(-12.3,28.0);
		std::vector<Coordinate2D<double>> test_path;

		Coordinate2D<double> path0(0.0,12.3);
...
//	void generatePath(const ScreenCoordinate& dest, double speed);
	TEST(generatePath){
		...
		MovingEffect test_effect(s_loc);
		Coordinate2D<double> d_loc(0.0, 1.0);
...
//	ScreenCoordinate getNextStep() const;
	TEST(moving){
		Coordinate2D<double> s_loc(0.0,0.0);
		Coordinate2D<double> path0(0.0,1.0);
		Coordinate2D<double> path1(1.0,1.0);
		Coordinate2D<double> path2(3.1244545663, -0.111111);
		std::vector<Coordinate2D<double>> test_path = {path2, path1, path0};
		...
		while(!test_path.empty()){
			Coordinate2D<double> step_loc = test_path.back();
			test_path.pop_back();
